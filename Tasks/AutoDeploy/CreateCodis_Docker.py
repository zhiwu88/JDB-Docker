...
def check(codisName, xyHostList, yzHostList):
  ...
  checkExists(cdscodisName, xyHostList, yzHostList)
...
def rsyncXyRedis(xyHostList):
  for xyHost in xyHostList:
    ret = commands.getoutput("rsync -avzp deply_codis_xy/monitor root@%s:/home/work/opbin/" % xyHost)
    print ret
    ret = commands.getoutput("rsync -avzp crontab_work root@%s:/tmp" % xyHost)
    print ret
    ret = commands.getoutput("rsync -avzp deply_codis_xy/codis_6*  deply_codis_xy/codis_7* root@%s:/home/work/" % xyHost)
    print ret
    ret = commands.getoutput("rsync -avzp deply_codis_xy/startCodis.sh root@%s:/home/work/" % xyHost)
    print ret
...
def rsyncXyDashboard(xyHostList, codisName):
  for xyHost in xyHostList:
    ret = commands.getoutput("rsync -avzp deply_codis_xy/dashboard root@%s:/home/work/" % xyHost)
    print ret
...
def rsyncXyProxy(xyHostList, codisName):
  for xyHost in xyHostList:
    ret = commands.getoutput("rsync -avzp deply_codis_xy/proxy_9100 root@%s:/home/work/" % xyHost)
    print ret
    ret = commands.getoutput("rsync tmp/config.ini-%s root@%s:/home/work/proxy_9100/conf/config.ini" % (codisName, xyHost))
    print ret
    ret = commands.getoutput("rsync -avzp deply_codis_xy/proxy_9200 root@%s:/home/work/" % xyHost)
    print ret
...
def rsyncYzRedis(yzHostList):
  for yzHost in yzHostList:
    ret = commands.getoutput("rsync -avzp deply_codis_yz/monitor root@%s:/home/work/opbin/" % yzHost)
    print ret
    ret = commands.getoutput("rsync -avzp crontab_work root@%s:/tmp" % yzHost)
    print ret
    ret = commands.getoutput("rsync -avzp deply_codis_xy/codis_6*  deply_codis_xy/codis_7* root@%s:/home/work/" % yzHost)
    print ret
    ret = commands.getoutput("rsync -avzp deply_codis_yz/startCodis.sh root@%s:/home/work/" % yzHost)
    print ret
...
def rsyncYzDashboard(yzHostList, codisName):
  for yzHost in yzHostList:
    ret = commands.getoutput("rsync -avzp deply_codis_yz/dashboard root@%s:/home/work/" % yzHost)
    print ret
...
def rsyncYzProxy(yzHostList, codisName):
  for yzHost in yzHostList:
    ret = commands.getoutput("rsync -avzp deply_codis_yz/proxy_9100 root@%s:/home/work/" % yzHost)
    print ret
    ret = commands.getoutput("rsync tmp/config.ini-%s root@%s:/home/work/proxy_9100/conf/config.ini" % (codisName, yzHost))
    print ret
    ret = commands.getoutput("rsync -avzp deply_codis_yz/proxy_9200 root@%s:/home/work/" % yzHost)
    print ret
...
def startXyDashboard(xyHostList):
  for xyHost in xyHostList:
    ret = commands.getoutput("ssh root@%s 'su - work -c \"source /etc/profile && cd /home/work/dashboard && sh load.sh start\"'" % xyHost)
    print ret
...
def createSh(codisName, xyHostList, yzHostList):
  ...
  #print ret
  ret = commands.getoutput("scp %s root@%s:/home/work/dashboard" % (shellFileName, xyHostList[0]))
  print ret
  ret = commands.getoutput("ssh root@%s 'chown -R work:work /home/work/dashboard/'" % xyHostList[0])
  print ret
