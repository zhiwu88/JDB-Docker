...
def main():
  ...
  with open("tmp/" + codisName + "-proYz.sh", "w") as yzFile:
    ...
    for group in groups:
      for server in group["servers"]:
        if server["type"] == "slave" and server["addr"].startswith("10.") and server["addr"][0:-1].endswith("600"):
  ...
  with open("tmp/" + codisName + "-proXy.sh", "w") as xyFile:
    ...
    for group in groups:
      for server in group["servers"]:
        if server["type"] == "master" and server["addr"].startswith("10.") and server["addr"][0:-1].endswith("600"):
